...
#### Input parameters: coutx files and name for output file ####
parser = args.ArgumentParser(description = 'Basic quality checks for row data: coutc, coutb and coutt tables: \n')

parser.add_argument('--cout', help = 'root for *.cout*.csv tables')
parser.add_argument('--out', help = 'root for output, default taken for cout', default = 'None')
...
if not os.path.isfile(rinput + '.coutc.csv') or not os.path.isfile(rinput + '.coutt.csv') or not os.path.isfile(rinput + '.coutb.csv'):
    sys.exit('coutc/t/b file not found')
...
mtexpr = {d: data[d].loc[[('__chrM' in idx or 'mt-' in idx) for idx in data[d].index]].sum() for d in data}
...
saveall = {'rawdata': data, 'row-QC': df}
pickle.dump(saveall, open(output + '.pickle', 'w') )
